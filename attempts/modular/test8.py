...
def diagnostic_process_image(img, projMgr):
    imgFtr = ImageFilters(projMgr.camCal, debug=True)

    # Run the functions
    imgFtr.imageQ(img)
    imgFtr.applyFilter4()
    imgFtr.horizonDetect()
    projMgr.findInitialRoadCorners(imgFtr)

    # assemble the screen
    diagScreen = np.zeros((1080, 1920, 3), dtype=np.uint8)
    diagScreen[0:540, 0:960] = cv2.resize(projMgr.diag2.astype(np.uint8), (960,540), interpolation=cv2.INTER_AREA) 
...
def process_diagnostic_image(image):
    ...
    result = diagnostic_process_image(image, projMgr)
    return result

camCal = CameraCal('camera_cal', 'camera_cal/calibrationdata.p')
projMgr = ProjectionManager(camCal, debug=True)
